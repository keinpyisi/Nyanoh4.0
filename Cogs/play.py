...
async def searchPlaylist(self, ctx, args):
    ...
    videoCount = int(len(Playlist(args).videos))
...
class CogPlay(commands.Cog):
    ...
    async def play(self, ctx, *args):
        ...
        # Youtube Playlist
            ...
            if args is None: return
         # Youtube Playlist
        elif args.startswith("https://youtube.com/playlist"): 
            args = await searchPlaylist(self, ctx, args)
            if args is None: return
        ...
        # YouTube video
        elif args.startswith("https://m.youtube.com/watch"):
            await ctx.send("<:YouTubeLogo:798492404587954176> Searching...", delete_after=10)
            videoid=video_id(args)
            
            # Check if the link exists
            track = await self.bot.wavelink.get_tracks("https://www.youtube.com/watch?v="+videoid)
            args = track[0]
            if track is None:
                return await ctx.send(f"{self.bot.emojiList.false} {ctx.author.mention} The YouTube link is invalid!")
